{-# LANGUAGE LambdaCase, TypeSynonymInstances, CPP #-}
...
instance Applicative Id where pure  = return
                              (<*>) = liftM2 id
...
instance Functor (State state) where fmap = liftM

instance Applicative (State state) where pure  = return
                                         (<*>) = liftM2 id
...
#if __GLASGOW_HASKELL__ >= 806
instance MonadFail m => MonadFail (StateT state m) where
...
foldList alg = \case x:s -> cons alg x $ foldList alg s
                     _ -> nil alg
...
list0 = foldList intAlg [1..8]
list1 = foldr (+) 0 [1..8]
list2 = foldList' intAlg [1..8]
...
data BinSig a btree = BinSig {empty_ :: btree, 
                              fork :: a -> btree -> btree -> btree}
          
foldBin :: BinSig a btree -> Bintree a -> btree
foldBin alg Empty               = empty_ alg
foldBin alg (Fork a left right) = fork alg a (foldBin alg left)
                                             (foldBin alg right)        

foldBinM :: BinSig a btree -> Bintree a -> Id btree
foldBinM alg Empty               = return $ empty_ alg
foldBinM alg (Fork a left right) = liftM2 (fork alg a) (foldBinM alg left)
                                                       (foldBinM alg right)
         
btree1 = Fork 6 (Fork 7 (Fork 11 (leaf 55) $ leaf 33) $ Empty) $ leaf 9
         -- > 6(7(11(55,33),),9)

sumBA = BinSig 0 (\i j k ->i+j+k)        

btree2 = foldBin sumBA btree1        
btree3 = (run . foldBinM sumBA) btree1          -- > 121        
btree4 = foldBin sumBA <<= btree1 :: Bintree Int 
btree5 = run . foldBinM sumBA <<= btree1 :: Bintree Int 
                                                -- > 121(106(99(55,33),),9)
...
data TreeSig a tree = TreeSig {var_ :: a -> tree,
                               fun :: a -> [tree] -> tree}

foldTree :: TreeSig a tree -> Tree a -> tree
foldTree alg (V a)    = var_ alg a
foldTree alg (F a ts) = fun alg a $ map (foldTree alg) ts

-- preordLA(t) lists the nodes of t in preorder.

preordLA :: TreeSig a [a]
preordLA  = TreeSig {var_ = single, fun = \a ass -> a:concat ass}

-- postordLA(t) lists the nodes of t in postorder.

postordLA :: TreeSig a [a]
postordLA  = TreeSig {var_ = single, fun = \a ass -> concat ass++[a]}

sumA :: Num a => TreeSig a a
sumA = TreeSig id $ \a as -> a+sum as
...
preordTA :: TreeSig a (State Int (Tree Int))
preordTA = TreeSig {var_  = \_    -> do i <- nextPos; return $ V i,
                    fun   = \_ ms -> do i <- nextPos; ts <- sequence ms
                                        return $ F i ts}
...
tree5 = foldTree preordLA tree1         -- > [1,2,2,3,-1,-2,4,-3,5]

tree6 = foldTree postordLA tree1        -- > [3,-1,2,-2,2,-3,5,4,1]
...
instance Applicative Dist where pure  = return
                                (<*>) = liftM2 id
