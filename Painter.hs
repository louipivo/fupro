{-# LANGUAGE FlexibleInstances, LambdaCase, TypeFamilies, CPP #-}

module Painter where

--------------------------------------- Copyright (c) Peter Padawitz, March 2018
...
--kfold f states (a:as) = kfoldl f (concatMap (f a) states) as  
--kfold _ states _      = states    
 
ktest = length$kfold(\a st ->[st,st])[0][1..6]          -- = 2^6       
...
fixptM :: Eq a => (a -> Maybe a) -> a -> Maybe a
fixptM step a = do b <- step a
                   if a == b then Just a else fixptM step b

-- READ, WRITE AND DRAW

readFileContinue :: a -> String -> (String -> IO a) -> IO a
readFileContinue err file act = do str <- readFile file `catch` handler
                                   if null str 
...
readFileAndDo = readFileContinue ()
...
instance MonadPlus Parser where mzero = P $ const Nothing
...
instance Functor Parser where fmap f m = do x <- m; return $ f x
...
parseBool :: Tree String -> Maybe Bool
parseBool = parse bool . root

parseInt :: Tree String -> Maybe Int
parseInt = parse int . root

some,many,nelist,list :: Parser a -> Parser [a]
some p   = do a <- p; fmap (a:) $ many p
many p   = some p `mplus` return []
nelist p = p >>= \a -> msum [tchar ',' >> fmap (a:) (nelist p), 
                             return [a]]
list p   = tchar '[' >> msum [tchar ']' >> return [],
                              do as <- nelist p; tchar ']'; return as]

identifier :: Parser String 
identifier = liftM2 (:) letter $ many 
                               $ sat (`notElem` "(){}<>=!&|+-*/^.,:; \n\t")
...
function,quoted :: Parser String
function = quoted `mplus` some (sat (`notElem` " \n\t()[]{},;<>=+-*/\\^:$#.|"))
quoted   = char '"' >> do x <- many $ nchar '"'; char '"'; return x

bool :: Parser Bool
bool = msum [string "true"  >> return True,
             string "false" >> return False]
             
relation :: Ord a => Parser (a -> a -> Bool)
relation = msum [string "==" >> return (==),
                 string "/=" >> return (/=),
                 string "<=" >> return (<=),
                 string ">=" >> return (>=),
                 string "<"  >> return (<),
...
term' (a:as) = do t <- term' as
                  ts <- many $ tstring a >> term' as
                  return $ if null ts then t else F a $ t:ts
term' _      = msum [tree,
                     do n <- token int; return $ F (show n) [],
                     token var,
                     token hovar >>= curryrest,
                     tuple >>= curryrest,
                     termList >>= curryrest,
...
var = do x <- msum $ map char "xyz"
         s <- many $ mplus letter digit
         return $ V $ x:s

hovar = do f <- msum $ map char "FGH"
           s <- many $ mplus letter digit
           return $ V $ f:s
                 
tree = msum [tchar 'V' >> fmap V (token quoted),
             tchar 'F' >> do x <- token quoted; F _ ts <- termList
                             return $ F x ts]

tuple = do tchar '('; msum [tchar ')' >> return (F "()" []),
                            do abs <- abstraction
                               ps <- many $ do tchar ','; abstraction
                               tchar ')'; return $ F "&#955;" $ abs:ps,
                            do op <- getOp; guard $ op /= "-"; t <- term
                               tchar ')'; return $ F "rsec" [F op [],t], 
...
abstraction,termList :: Parser (Tree String)

abstraction = do p <- term; msum [tstring "->" >> do t <- term
                                                     return $ F "->" [p,t],
                                  tstring "|" >> do b <- term; tstring "->"
                                                    t <- term
                                                    return $ F "->" [p,b,t]]

termList = msum [tchar '[' >> do t <- term; tstring ".."; u <- term; tchar ']'
                                 return $ F "range" [t,u],
                 do ts <- list term; return $ F "[]" ts]
                 
fields :: Parser [Tree String]            -- Field entries are preceded by "FF".
fields = tchar '{' >> do ts <- fs; tchar '}'; return ts
         where fs = do field <- token function; tchar '='; t <- term
                       let val = F ("FF"++field) [t]
                       msum [tchar ',' >> fmap (val:) fs, 
                             return [val]]

curryrest :: Tree String -> Parser (Tree String)
curryrest t = msum [tuple >>= curryrest . applyTo,
                    termList >>= curryrest . applyTo,
                    do ts <- fields; F x [] <- return t; return $ F x ts,
                    do x <- token function; curryrest $ applyTo $ F x [],
                    term >>= curryrest . applyTo,
                    return t] where
...
instance MonadTrans MaybeT where 
                    ...
                    liftT = MaybeT . return     -- :: Maybe a -> MaybeT m a
...
reduce file rules = readFileAndDo file f where
                    ...
                    f str = case runP termEqs str of 
                            Just ((t,eqs),_) 
                              -> drawList redfile svg $ reduction t $ 
                                          Just:map applyEq eqs ++ rules
...
applyEq eqs t = msum $ map f eqs where 
               f (left,right) = msum [do sub <- match t left
                                         Just $ right >>> sub,
                                      do F f ts <- Just t
                                         sub <- match (F f []) left
                                         Just $ F "$" [right >>> sub,F "()" ts],
                                      do F "$" [t,u] <- Just t
                                         sub <- match t left
                                         Just $ F "$" [right >>> sub,u]]
...
mkBool True = Just $ mkTrue
mkBool _    = Just $ mkFalse
...
eval,flatten,impl :: Rules String
...
eval (F "ite" [t,u,v]) | isJust b         = Just $ if fromJust b then u else v
                                            where b = parseBool t
eval (F "range" [t,u]) | isJust i && isJust k  = mkList $ map f [fromJust i..
                                                                 fromJust k]
                                            where i = parseInt t; k = parseInt u
                                                  f i = F (show i) [] 
eval (F "$" [t,F "()" [u]])               = Just $ F "$" [t,u]
eval (F "$" [F "&#955;" [F "->" [p,t]],u]) | isJust sub 
                                          = Just $ t>>>fromJust sub
                                            where sub = match u p
eval (F "$" [F "&#955;" ts@(_:_),u])      = Just $ F "CASE" cases where
                       cases = [fromJust t | t <- map f ts, isJust t]
                       f (F "->" ts) = msum [do [p,t] <- Just ts
                                                sub <- match u p
                                                Just $ F "->" [mkTrue,t>>>sub],
                                             do [p,b,t] <- Just ts
                                                sub <- match u p
                                                Just $ F "->" [b>>>sub,t>>>sub]]
eval (F "CASE" (F "->" [F "true" [],t]:_))   = Just t
eval (F "CASE" (F "->" [F "false" [],_]:ts)) = Just $ F "CASE" ts
eval (F "CASE" [])                           = Just $ F "undefined" []
...
eval (F "#" ts@(_:_:_:_))                    = Just $ F "$" [last ts,
                                                             F "#" $ init ts]
eval (F "#" [t,u])                           = Just $ F "$" [u,t]
eval _                                       = Nothing
...
impl = \case F "\\/" ts -> if b then mkBool True 
                           else Just $ F "==>" [norm "/\\" prem,norm "or" conc]
                           where (prem,conc,b) = split ts
             t@(F "/\\" ts) | t /= u -> Just u where u = norm "/\\" ts
             _ -> Nothing
...
-- XCTL = XPath and CTL: subsets of and binary relations between tree nodes
...
xalg :: Tree String -> XCTL [Node] (Node -> [Node])
xalg t = XCTL {true_     = nodes t,
               false_    = [],
...
foldSet :: XCTL nodeSet nodeRel -> Tree String -> Maybe nodeSet
foldSet alg = f where 
       f = \case F "true" []        -> Just $ true_ alg
                 F "false" []       -> Just $ false_ alg   
                 F a []             -> Just $ atom alg (== a)   
                 F "rsec" [F op [],F a []] | isJust i && isJust rel      
                                    -> Just $ atom alg p
                                       where i = parse int a                                              
                                             ...
                                             p b = isJust k && fromJust rel 
                                                   ...
                                                   where k = parse int b
                 F "/\\" ts@(_:_)   -> fmap (foldl1 $ and_ alg) $ mapM f ts
                 F "\\/" ts@(_:_)   -> fmap (foldl1 $ or_ alg) $ mapM f ts
                 F "&exist;" [t,u]  -> liftM2 (exists alg) (foldRel alg t) $ f u
                 F "&forall;" [t,u] -> liftM2 (forall alg) (foldRel alg t) $ f u
                 F "not" [t]        -> fmap (not_ alg) $ f t
                 F "EX" [t]         -> fmap (ex alg) $ f t
                 F "AX" [t]         -> fmap (ax alg) $ f t
                 F "EF" [t]         -> fmap (ef alg) $ f t
                 F "AF" [t]         -> fmap (af alg) $ f t
                 F "EG" [t]         -> fmap (eg alg) $ f t
                 F "AG" [t]         -> fmap (ag alg) $ f t
                 _                  -> Nothing

foldRel :: XCTL nodeSet nodeRel -> Tree String -> Maybe nodeRel
foldRel alg = f where 
         f = \case F "+" ts@(_:_) -> fmap (foldl1 $ par alg) $ mapM f ts
                   F "/" ts@(_:_) -> fmap (foldl1 $ seq_ alg) $ mapM f ts
                   F "clos" [t]   -> fmap (closure alg) $ f t
                   F "inv" [t]    -> fmap (inv alg) $ f t
                   F ">>" [t,u]   -> liftM2 (restrict alg) (f t) $ foldSet alg u
                   F x []         -> case x of 
                                          "self"       -> Just $ self alg
...
-- SQL (see https://de.wikipedia.org/wiki/SQL,  
--          https://de.wikipedia.org/wiki/Relationale_Algebra,
--          https://en.wikipedia.org/wiki/Relational_algebra)
 
data Table a = Table {attrs :: [String], vals :: String -> [a]} 

emptyTab = Table [] $ const []

type Row a = [(String,a)]

instance Show a => Show (Table a) where
            show (Table ats f) = init ("tab{"++drop 5 (concatMap g ats)) ++"}"
                                 where g x = "\n    "++x++" = "++show (f x)++","

-- mkRows t transforms t into the corresponding list of rows of a matrix 
-- representing t.

mkRows :: Monoid a => Table a -> [Row a]
mkRows (Table [] _)  = []
mkRows (Table ats f) = map g [0..max-1]
                       where g i = map (\x -> (x,h x i)) ats
                             h x i = if i < length (f x) then f x!!i else mempty
                             max = maximum $ map (length . f) ats
                           
mkTab :: [Row a] -> Table a
mkTab (row:rows) = Table (union ats $ map fst row) g
                   where Table ats f = mkTab rows
                         g x = case lookup x row of Just a -> a:f x; _ -> f x
mkTab _          = Table [] $ const []

-- lookupTab x ts takes the table t with name x from a list of trees 
-- representing tables and transforms t into an object of type Table a.

lookupTab :: Read a => String -> [Tree String] -> Maybe (Table a)
lookupTab x ((F ('F':'F':y) [F "tab" ts]):us) = if x == y then trees2cols ts
                                                          else lookupTab x us
lookupTab _ _ = Nothing

trees2cols :: Read a => [Tree String] -> Maybe (Table a)
trees2cols ts = do pairs <- mapM tree2col ts
                   let (ats,vals) = unzip pairs
                   Just $ Table ats $ fold2 update (const []) ats vals

tree2col :: Read a => Tree String -> Maybe (String,[a])
tree2col (F ('F':'F':atr) [F "[]" ts@(_:_)]) = do as <- mapM tree2val ts
                                                  Just (atr,as)
tree2col _ = Nothing      

tree2val :: Read a => Tree String -> Maybe a
tree2val (V x)    = tree2val $ F x []
tree2val (F x []) = case reads x of [(a,[])] -> Just a; _ -> Nothing
                                              
tree2val _ = Nothing
            
data Val = N String | I Int deriving (Eq,Ord)
...
instance Monoid Val where mempty = I 0
                          mappend (I i) (I k) = I $ i+k
                          mappend _ _         = I 0

instance Read Val where readsPrec _ s = [(I i,s) | (i,s) <- reads s] ++ 
                                        [(N s,[]) | isJust $ parse identifier s]

instance Show Val where show (N name) = name
                        show (I i)    = show i

data RelAlg table row a = 
     RelAlg {project         :: [String] -> table -> table,
...
relAlg :: (Eq a,Ord a,Read a,Show a,Monoid a) => RelAlg (Table a) (Row a) a
relAlg = RelAlg {project   = project, 
                 select    = select,
                 prod      = prod,
                 div_      = div_,
                 njoin     = njoin,
                 ejoin     = ejoin,
                 getAttrs  = attrs,
                 getVal    = \x -> fromJust . lookup x,
                 getTable  = getTable,
                 getfromDB = getfromDB,
                 putHere   = putHere,
                 putPict   = putPict}
           where project ats tab = mkTab [filter p row | row <- mkRows tab]
                                   where p (x,_ ) = x `elem` ats
                 select pred tab = mkTab [row | row <- mkRows tab, pred row]
                 ptab p tab1 tab2 = mkTab [row1++row2 | row1 <- mkRows tab1, 
                                                        row2 <- mkRows tab2,
                                                        p row1 row2]
                 prod tab1@(Table ats1 f1) tab2@(Table ats2 f2)
                                 = ptab (const $ const True) tab1 tab3 
                                   where tab3 = Table (ats3++primed) f
                                         ats3 = diff ats2 ats1
                                         primed = map (++ "'") $ meet ats1 ats2
                                                          -- attribute renaming
                                         f at | at `elem` ats3   = f2 at
                                              | at `elem` primed = f2 $ init at
                 div_ tab1 tab2 = mkTab [row | row <- rows, 
                                               all (p row) $ mkRows tab2] 
                               where rows = union [] $ mkRows $ project ats tab1
                                     ats = attrs tab1 `diff` attrs tab2
                                     p row row2 = sort (row++row2) `elem` rows1
                                     rows1 = map sort $ mkRows tab1
                 njoin = ptab p 
                     where p ((x,a):row1) row2 = case lookup x row2 of 
                                                 Just b -> a == b && p row1 row2
                                                 _ -> p row1 row2
                           p _ _ = True
                 ejoin x y = ptab p
                     where p ((z,a):row1) row2 =
                                  if z == x then case lookup y row2 of 
                                                      Just b -> a == b
                                                      _ -> p row1 row2
                                            else p row1 row2
                           p _ _ = True
                 getTable file = readFileContinue emptyTab file act 
                     where act str = case parse term str of 
                                          Just (F "tab" ts) 
                                            -> case trees2cols ts of 
                                                    Just tab -> return tab
                                                    _ -> return emptyTab
                                          _ -> return emptyTab
                 getfromDB tab file = readFileContinue emptyTab file act 
                     where act db = case parse term db of 
                                         Just (F "db" ts) 
                                           -> case lookupTab tab ts of 
                                                   Just tab -> return tab
                                                   _ -> return emptyTab
                                         _ -> return emptyTab
                 putHere tab = do putStrLn $ show tab; return tab
                 putPict tab file = do writeFile ("../ExpanderLib/"++file) $
                                                 pictTerm tab file
                                       return tab

foldTable ::  (Ord a,Read a,Monoid a) => RelAlg table row a -> Tree String 
                                                            -> MaybeT IO table
foldTable alg = f where
          f = \case F "project" [F "[]" ts,t] | all isLeaf ts
                                      -> fmap (project alg $ map root ts) $ f t
                    F "project" [t,u] | isLeaf t 
                                      -> fmap (project alg [root t]) $ f u
                    F "select" [p,t]  -> do tab <- f t
                                            let ats = getAttrs alg tab
                                            pred <- liftT $ foldPred alg ats p 
                                            return $ select alg pred tab
                    F "prod" [t,u]    -> liftM2 (prod alg) (f t) $ f u
                    F "div" [t,u]     -> liftM2 (div_ alg) (f t) $ f u
                    F "tjoin" [p,t,u] -> f $ F "select" [p,F "prod" [t,u]]
                    F "njoin" [t,u]   -> liftM2 (njoin alg) (f t) $ f u
                    F "ejoin" [x,y,t,u]   
                                      -> liftM2 ((ejoin alg) (root x) $ root y)
                                                             (f t) $ f u
                    F "here" [t]      -> do tab <- f t; lift $ putHere alg tab
                    F "pict" [t,u] | isLeaf t
                                      -> do tab <- f t
                                            lift $ putPict alg tab $ root u
                    F "fromDB" [t,u] | isLeaf t && isLeaf u
                                      -> lift $ getfromDB alg (root t) $ root u
                    t | isLeaf t      -> lift $ getTable alg $ root t
                    _                 -> liftT Nothing

foldPred :: (Ord a,Read a,Monoid a) => RelAlg table row a -> [String] 
                                    -> Tree String -> Maybe (row -> Bool)
foldPred alg ats = f where
                f = \case F "\\/" ts@(_:_) -> fmap (foldl1 (|||)) $ mapM f ts
                          F "/\\" ts@(_:_) -> fmap (foldl1 (&&&)) $ mapM f ts
                          F "not" [t]      -> fmap (fmap not) $ f t
                          F op [t,u] | isJust rel 
                                           -> liftM2 (liftM2 rel') (ff t) $ ff u
                                              where ff = foldFun alg ats
                                                    rel = parse relation op
                                                    rel' = fromJust rel
                          _                -> Nothing
                             
foldFun :: (Ord a,Read a,Monoid a) => RelAlg table row a -> [String] 
                                   -> Tree String -> Maybe (row -> a)
foldFun alg ats = f where
                  f = \case V x    -> f $ F x []
                            F x [] -> Just $ if x `elem` ats then getVal alg x 
                                                             else const $ read x
                            F "+" ts@(_:_:_) 
                                   -> fmap (foldl1 $ liftM2 mappend) $ mapM f ts
                            _      -> Nothing
                            
-- compTable query parses the term query and folds the resulting table.

compTable :: String -> IO ()
compTable query = case parse term query of 
                       Just tree -> do tab <- runMT $ foldTable relAlg tree
                                       if isJust (tab :: Maybe (Table Val)) 
                                       then return () else putStrLn "NO TABLE"
                       _         -> putStrLn "NO PARSE"

-- dbPict file parses the database term db stored in file, transforms db into a
-- list of widget terms representing the tables of db according to pictTerm and
-- saves them to ExpanderLib/file from where Expander2 may fetch them and 
-- display them graphically.

dbPict :: String -> IO ()
dbPict file = readFileAndDo file act where
     act db = case parse term db of 
              Just (F "db" ts) | all (fst . isTable) ts
                -> do tabs <- mapM (f . isTable) ts
                      let terms = zipWith pictTerm tabs $ map (snd . isTable) ts
                      writeFile ("../ExpanderLib/" ++ file)
                                $ drop 4 $ concatMap ("\n<+>"++) terms
                   where isTable = \case (F ('F':'F':x) [_]) -> (True,x)
                                         _                   -> (False,"")
                         f (_,tab) = getfromDB relAlg tab file :: IO (Table Val)
              _ -> putStrLn "NO DATABASE"

-- pictTerm tab name constructs a widget term from tab. This term is interpreted
-- by Expander2 as the graphical representation of tab. 
-- pictTerm is used by tabPict and dbPict.
                         
pictTerm :: (Show a,Monoid a) => Table a -> String -> String
pictTerm tab name = "shelf(1)[text$" ++ name ++",RGB 221 221 255$circ$12,matrix" 
                    ++ (f $ show $ concat $ zipWith g inds rows) ++ "]"
                    where rows = mkRows tab
                          inds = [1..length rows]
                          f ('"':str) = f str
                          f (x:str)   = x:f str
                          f _         = ""
                          g i row = [(i,x,a) | (x,a) <- row]
                       
-- Examples 

db1    = dbPict "database"

drawDB = drawTermC "database"

drawT1 = drawTerm "table1"
                         
table0 = compTable "pict(table1,table1)"

table1 = compTable "here(table1)"       -- tab{name = [Hilde,Inge,Robin],
                                        --     age = [81,85,72],
                                        --     children = [1,3,2]}

table2 = compTable "pict(fromDB(table2,database),table2)"       
                                        -- tab{name = [Robin,Inge,Markus],
                                        --     wife = [Hanna,Isolde,Hilde],
                                        --     age = [72,85,72]}


table3 = compTable "pict(select(age<78+2\\/name==Hilde,table1),table3)"
                                        -- tab{name = [Hilde,Robin],
                                        --     age = [81,72],
                                        --     children = [1,2]}

table4 = compTable "pict(project([name,age],table1),table4)"    
                                        -- tab{name = [Hilde,Inge,Robin],
                                        --     age = [81,85,72]}

table5 = compTable "pict(project([name,children],table1),table5)"       
                                        -- tab{name = [Hilde,Inge,Robin],
                                        --     children = [1,3,2]}

table6 = compTable "here(pict(njoin(table1,fromDB(table2,database)),table6))" 
                                        -- tab{name = [Inge,Robin],
                                        --     age = [85,72],
                                        --     children = [3,2],
                                        --     wife = [Isolde,Hanna]}
   
table7 = compTable "here(fromDB(table7,database))"      
                                        -- tab{A = [1,4,7],
                                        --     B = [2,5,8],
                                        --     C = [3,6,9],
                                        --     D = [4,7,0]}
   
table8 = compTable "here(fromDB(table8,database))"      
                                        -- tab{A = [1,7],
                                        --     F = [2,8],
                                        --     G = [3,9]}
...
query9  = "here(njoin(fromDB(table7,database),fromDB(table8,database)))"
                                        
query9' = "here(ejoin(C,G,fromDB(table7,database),fromDB(table8',database)))"

table9 = compTable query9               -- tab{A = [1,7],
                                        --     B = [2,8],
                                        --     C = [3,9],
                                        --     D = [4,0],
                                        --     F = [2,8],
...
query10 = "here(pict(div(fromDB(completed,database)," ++
                        "fromDB(dbProject,database)),table10))"

table10 = compTable query10             -- tab{Student = [Fred,Sarah]}

query13 = "here(pict(div(fromDB(table11,database)," ++
                        "fromDB(table12,database)),table13))"
                               
table13 = compTable query13             -- tab{Vater = [Moritz],
                                        --     Mutter = [Melanie]}

query15 = "here(prod(project([A,B,C],fromDB(table7,database))," ++
                    "fromDB(table14,database)))" 
                   
table15 = compTable query15             -- tab{A = [1,1,4,4,7,7],
                                        --     B = [2,2,5,5,8,8],
                                        --     C = [3,3,6,6,9,9],
                                        --     D = [3,6,3,6,3,6],
                                        --     E = [1,2,1,2,1,2]}
                
query16 = "here(tjoin(B<D,project([A,B,C],fromDB(table7,database))," ++
                         "fromDB(table14,database)))" 

table16 = compTable query16             -- tab{A = [1,1,4],
                                        --     B = [2,2,5],
                                        --     C = [3,3,6],
                                        --     D = [3,6,6],
                                        --     E = [1,2,2]}

drawQ16 = mkTree "query16" query16

query19 = "here(div(fromDB(table17,database),fromDB(table18,database)))"

table19 = compTable query19             -- tab{A = [1],
                                        --     B = [2]}
...
drawNodeSet file form = readFileAndDo file act where
                   act tstr = case phi of Just set -> draw1 (t,set)
                                          _ -> draw2 $ F "NO NODESET" []
                              where t = fromJust $ parse term tstr
                                    phi = (parse term >=> foldSet (xalg t)) form
                                    file' = file ++ "_set"
                                    draw1 = drawSingle 10 40 file' svgTreeSet
                                    draw2 = drawSingle 10 40 file' svgTree
...
-- mkPTree admits multiline node labels and interprets '%' as a line break
-- symbol.
...
svgTreeC spread t = (f red pt,round2 $ treeSize pt)
    where pt = mkPTree spread (50,20) t
          f :: RGB -> PTree -> String
...
combine :: [Curve] -> Curve
combine cs@(c:_) = c {colors = concatMap colors cs, modes = concatMap modes cs,
                      paths = concatMap paths cs} 
combine _        = emptyC ""
...
-- or Heinrich M�ller, B-Spline-Technik, Vorlesung Geometrisches Modellieren 
...
instance Read RGB where 
         readsPrec _ s = [(RGB r g b,s) | ("RGB",s) <- lex s, (r,s) <- reads s,
                                          (g,s) <- reads s, (b,s) <- reads s] ++
                         [(rgb c,s)     | (c,s) <- lex s, c `elem` 
                         words "red green blue yellow magenta cyan black white"]
                  
rgb = \case "red" -> red; "green" -> green; "blue" -> blue; "yellow" -> yellow
            "magenta" -> magenta; "cyan" -> cyan; "black" -> black
            "white" -> white
